/**
 * Управляє грою, запам'ятовує якісь карти були відкриті, управляє колодою і вважає кількість спроб.
...
class GameManager {
    ...
    selectCard(card) {
        if (card == this.#firstCard) return; // якщо вдруге натиснули на одну й ту ж саму карту, нічого не робимо (метод далі не виконуємо)
        ...
        // Якщо вибрана одна карта запам'ятовуємо її і чекаємо на другу
        if (this.#firstCard == null) {
            ...
            if (this.#firstCard.imagePath === card.imagePath) {
                this.#deck.removeCard(this.#firstCard); // прибираємо карти з колоди (вони залишаються у DOM дереві)
